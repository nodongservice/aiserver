...
### 기능 2. 퀵 맞춤 일자리 추천 (전체 후보 + 직무/거리 적합)

- 프론트는 별도 퀵공고 페이지에서 Spring Backend만 호출한다.
- Spring Backend는 AI ON 추천을 비동기 task로 관리하고, 100개 배치를 1개 단위 FastAPI 요청으로 나눠 부분 결과를 누적할 수 있다.
- FastAPI는 `limit`/`offset`을 안정적으로 처리하고, `limit=1` 요청도 일반 요청과 동일한 응답 구조로 반환한다.
- 후보 공고는 모집 중이고 마감일이 지나지 않은 공고 전체를 기준으로 빠르게 랭킹하고, 상위 1000개 안에서 페이지네이션한다.

1. AI 직무 적합도 토글 ON
    - 프론트에서 프로필 1개 선택(기본 프로필 최상단 노출)
    - Spring → FastAPI: 사용자 선택 프로필만 전달
    - FastAPI: 전체 후보 공고를 직무 적합도, 근무조건, 거주지-근무지 거리 기준으로 빠르게 랭킹한 뒤 상위 후보만 계산
    - FastAPI → Spring: 공고별 퀵 추천 점수와 산정 근거 반환
    - Spring → 프론트: 결과 전달
...
1. AI 스코어링 토글 ON
    ...
    - Spring → 프론트: 결과 전달
    - 프론트: 화면 필터 적용
    - 지도 추천은 전체 후보를 빠른 프로필 기반 점수로 랭킹하고, 상위 1000개 안에서 접근성/통근/공공데이터 정밀 점수 계산 후 `limit`/`offset`을 적용한다.
    - Spring Backend가 100개 배치를 1개 단위 FastAPI 요청으로 나눠 부분 결과를 누적할 수 있다.
    - 프론트는 부분 결과를 1개씩 반영하고 최대 1000개까지만 표시한다.
2. AI 스코어링 토글 OFF
    - FastAPI 호출 없음
    - Spring이 DB 공고 반환
    - 프론트가 화면 필터 적용
...
- 기능2: 직무 적합도, 근무조건, 거주지-근무지 거리 감점을 함께 적용한 퀵 추천 점수
...
1. 스코어링 API
    - `POST /api/v1/score/quick`
        - 입력: 선택 프로필 1개
        - 처리: 모집 중/마감 전 전체 후보 랭킹 + 직무/근무조건/거리 기반 퀵 추천 점수 계산
        - 출력: 공고 + `job_fit_score` + 근거
    - `POST /api/v1/score/map`
        - 입력: 선택 프로필 1개
        - 처리: 전체 후보 빠른 랭킹 + 상위 1000개 공고/공공데이터 조회 + 종합점수 계산 + 점수 정렬 후 페이지네이션
        - 출력: 공고 + 항목별 점수 + 총점 + 근거
2. 설명 생성 API(선택)
