...
- 후보 공고는 모집 중이고 마감일이 지나지 않은 공고 전체를 기준으로 빠르게 랭킹하고, 상위 100개 안에서 페이지네이션한다.
...
1. AI 스코어링 토글 ON
    ...
    - Spring → 프론트: 결과 전달
    - 프론트: 화면 필터 적용
    - 지도 추천은 전체 후보를 빠른 프로필 기반 점수로 랭킹하고, 상위 100개 안에서 접근성/통근/공공데이터 정밀 점수 계산 후 `limit`/`offset`을 적용한다.
...
1. 스코어링 API
    ...
    - `POST /api/v1/score/map`
        - 입력: 선택 프로필 1개
        - 처리: 전체 후보 빠른 랭킹 + 상위 100개 공고/공공데이터 조회 + 종합점수 계산 + 점수 정렬 후 페이지네이션
        - 출력: 공고 + 항목별 점수 + 총점 + 근거
2. 설명 생성 API(선택)
