...
def upsert_accessibility_gis_feature(
    ...
    MVP에서는 public_data_record_id 기준으로 기존 row를 비활성화하지 않고,
    같은 record_id + feature_type 조합이 있으면 업데이트합니다.

    주의:
    - 운영에서는 Spring migration/동기화 로직에서 관리하는 것을 권장합니다.
    - FastAPI의 이 함수는 개발/검증용입니다.
    """
